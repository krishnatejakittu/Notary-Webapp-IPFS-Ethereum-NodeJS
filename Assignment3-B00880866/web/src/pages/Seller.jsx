...
const Seller = () => {
  ...
  const ApproveAgreement = async () => {
    ...
    await Agreement.methods.approvedAgreement("SELLER", signature).send({
      from: accounts[0],
    });
  ...
  const DeclineAgreement = async () => {
    const accounts = await web3.eth.getAccounts();
    setIsLoading(true);
    await Agreement.methods.rejectAgreement("SELLER").send({
      from: accounts[0],
    });
    setIsLoading(false);
    agreementFetching();
    ...
    <Flex w="100vw" h="100vh" justify="center" align="center" flexDir="column">
      ...
      <Flex w="50%" mt="10" justify="space-evenly" align="center">
        ...
        <Button
          ...
          isLoading={isLoading}
          isDisabled={isLoading || status.isApprovedBySeller === "TRUE"}
        >
          Sign & Approve
        </Button>
        <Button
